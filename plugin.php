...
class ETPlugin_Sitemap extends ETPlugin
{
	...
	public function handler_conversationModel_createAfter($model, $conversation, $postId, $content)
	{
		...
		if (!file_exists(PATH_ROOT."/sitemap-index.xml") or filemtime(PATH_ROOT."/sitemap-index.xml") < time() - (C("plugin.Sitemap.cache") * 3600) - 200) {
			$this->action_create();
		}
	}
	...
	function autoSubmit()
	{
		if(C("plugin.Sitemap.google")) {
			$response = $this->cURL("http://www.google.com/webmasters/tools/ping?sitemap=".urlencode(C("esoTalk.baseURL")."sitemap-index.xml"));
		...
		if(C("plugin.Sitemap.bing")) {
			$response = $this->cURL("http://www.bing.com/ping?sitemap=".urlencode(C("esoTalk.baseURL")."sitemap-index.xml"));
